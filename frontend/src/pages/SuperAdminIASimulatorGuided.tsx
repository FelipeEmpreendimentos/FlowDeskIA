...
function vehicleLabel(vehicle: SimulatorVehicle): string {
  ...
  const extra = [vehicle.tipo_veiculo, vehicle.ano ? String(vehicle.ano) : null, vehicle.cor]
    .filter(Boolean)
    .join(" · ");
...
export function SuperAdminIASimulatorGuided() {
  ...
  }, [messages, quickReplies, responding]);
  ...
  async function resetConversation() {
    ...
    try {
      ...
      // Um novo session id também isola a nova conversa.
    }
    setSessionId(newId());
    setMessages([]);
    setQuickReplies([]);
  ...
  async function submitCustomerMessage(text: string, actionId?: string) {
    ...
    try {
      ...
      setQuickReplies(response.handoff ? [] : response.quick_replies ?? []);
  ...
  return (
    <div className="super-admin-page super-admin-ai-simulator">
      <header className="super-admin-page-header">
        <div>
          <span>Laboratório guiado</span>
          <h1>Simulador de IA</h1>
          <p>Teste o atendimento híbrido: opções rápidas quando o caminho é conhecido e IA para interpretar mensagens livres.</p>
        </div>
      ...
      <div className="ai-v2-write-warning">
        Os botões executam os mesmos fluxos que o futuro WhatsApp usará. Agendar, reagendar e cancelar continuam exigindo confirmação antes da escrita definitiva.
      </div>
      ...
      <section className="super-admin-ai-workspace">
        ...
        <section className={`wa-phone-shell super-admin-wa-shell ${!bootstrap ? "is-disabled" : ""}`} aria-label="WhatsApp simulado">
          ...
          <div className="wa-phone-encryption-note"><Icon name="lock" size={12} /> Fluxo guiado · clique ou escreva normalmente</div>

          <div className="wa-phone-messages">
            {!bootstrap && <div className="super-admin-ai-empty-chat"><Icon name="chat" size={30} /><strong>Escolha empresa e cliente</strong><span>Depois envie “Oi” para abrir o menu de atendimento.</span></div>}
          ...
          {quickReplies.length > 0 && !debug.handoff && (
            <div className="guided-quick-replies" aria-label="Opções rápidas">
              {quickReplies.map((option) => (
                <button
                  type="button"
                  className={`guided-quick-reply guided-quick-reply-${option.kind}`}
                  key={option.id}
                  disabled={responding}
          ...
          <form className="wa-phone-composer" onSubmit={sendTyped}>
            <textarea
              ...
              placeholder={debug.handoff ? "Atendimento transferido" : "Digite normalmente ou use as opções acima"}
